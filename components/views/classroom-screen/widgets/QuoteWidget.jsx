...
export function QuoteWidget({ isDark }) {
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex-1 flex flex-col items-center justify-center text-center relative min-h-0">
        ...
        <Button size="sm" variant="outline" onMouseDown={e => e.stopPropagation()} onClick={(e) => { e.stopPropagation(); setQuoteIdx((quoteIdx + 1) % QUOTES.length) }} className={`mt-2 h-7 text-xs ${isDark ? 'bg-white/10 border-white/20 text-white hover:bg-white/20' : ''}`}>
