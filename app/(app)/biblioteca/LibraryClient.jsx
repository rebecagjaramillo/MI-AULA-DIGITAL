...
export function LibraryClient({ serverItems }) {
  ...
  const remove = async (r) => { 
    ...
    try {
      await api('library/' + r.id, { method: 'DELETE' })
  ...
  const toggleFav = async (r) => { 
    try {
      await api('library/' + r.id, { method: 'PUT', body: JSON.stringify({ favorite: !r.favorite }) })
      router.refresh()
    } catch (e) { toast.error(e.message) }
  }
        ...
        <CardContent className="p-4 flex flex-wrap items-end gap-3">
          <div className="flex-1 min-w-[200px]">
            ...
            <div className="relative mt-1">
              ...
              <Input id="search" className="pl-9" placeholder="Título, descripción, etiquetas..." value={search} onChange={e => setSearch(e.target.value)} />
            </div>
          </div>
          <div className="min-w-[160px]">
            <Label htmlFor="filterType" className="text-xs font-semibold">Tipo</Label>
            <Select value={filterType} onValueChange={setFilterType}>
              <SelectTrigger id="filterType" className="mt-1"><SelectValue /></SelectTrigger>
              <SelectContent>
          ...
          <Button variant={filterFav ? 'default' : 'outline'} className={filterFav ? 'bg-amber-500 hover:bg-amber-600 text-white' : ''} onClick={() => setFilterFav(!filterFav)}>
              ...
              <Card key={r.id} className="border-slate-100 hover:shadow-md transition-shadow group overflow-hidden">
                <CardContent className="p-5">
                  <div className="flex items-start justify-between mb-2">
                    ...
                    <button onClick={() => toggleFav(r)} className={`text-lg leading-none transition-all ${r.favorite ? 'opacity-100 scale-110' : 'opacity-30 hover:opacity-60'}`} title="Favorito" aria-label="Favorito">⭐</button>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-1.5 mt-2 text-2xs text-slate-500">
                    {r.subject && <Badge variant="outline" className="text-2xs py-0">{r.subject}</Badge>}
                    {r.grade && <Badge variant="outline" className="text-2xs py-0">{r.grade}</Badge>}
                    {r.tags && r.tags.split(',').slice(0,3).map(t => <Badge key={t} variant="outline" className="text-2xs py-0">#{t.trim()}</Badge>)}
                  </div>
                  <div className="flex items-center gap-1.5 mt-4 pt-3 border-t border-slate-100">
                    <Button asChild size="sm" className="flex-1 bg-sky-500 hover:bg-sky-600 shadow-none text-white">
                      <a href={r.url} target="_blank" rel="noopener noreferrer" aria-label="Abrir documento">Abrir →</a>
                    </Button>
                    <Button size="icon" variant="ghost" className="h-8 w-8" onClick={() => openEdit(r)} aria-label="Editar documento"><Pencil className="w-3.5 h-3.5" /></Button>
                    <Button size="icon" variant="ghost" className="h-8 w-8 text-rose-500 hover:bg-rose-50" onClick={() => remove(r)} aria-label="Eliminar documento"><Trash2 className="w-3.5 h-3.5" /></Button>
                  </div>
                </CardContent>
              </Card>
            )
        ...
        <DialogContent className="sm:max-w-[560px]">
          ...
          <div className="space-y-3">
            <div>
              ...
              <Input id="title" className="mt-1" value={form.title} onChange={e => setForm({...form, title: e.target.value})} placeholder="Ej. Khan Academy - Fracciones" />
            </div>
            <div>
              <Label htmlFor="url" className="text-xs font-semibold">URL *</Label>
              <Input id="url" className="mt-1" value={form.url} onChange={e => setForm({...form, url: e.target.value})} placeholder="https://..." />
            </div>
            <div>
              <Label htmlFor="description" className="text-xs font-semibold">Descripción</Label>
              <Textarea id="description" className="mt-1 resize-none" rows={2} value={form.description} onChange={e => setForm({...form, description: e.target.value})} placeholder="¿Para qué sirve este recurso?" />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <Label htmlFor="subject" className="text-xs font-semibold">Materia</Label>
                <Input id="subject" className="mt-1" value={form.subject} onChange={e => setForm({...form, subject: e.target.value})} placeholder="Ej. Matemáticas" />
              </div>
              <div>
                <Label htmlFor="grade" className="text-xs font-semibold">Grado</Label>
                <Input id="grade" className="mt-1" value={form.grade} onChange={e => setForm({...form, grade: e.target.value})} placeholder="Ej. 5°" />
              </div>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <Label htmlFor="resource_type" className="text-xs font-semibold">Tipo</Label>
                <Select value={form.resource_type} onValueChange={v => setForm({...form, resource_type: v})}>
                  <SelectTrigger id="resource_type" className="mt-1"><SelectValue /></SelectTrigger>
                  <SelectContent>{RESOURCE_TYPES.map(t => <SelectItem key={t.value} value={t.value}>{t.icon} {t.label}</SelectItem>)}</SelectContent>
                </Select>
              </div>
              <div>
                <Label htmlFor="tags" className="text-xs font-semibold">Etiquetas (coma)</Label>
                <Input id="tags" className="mt-1" value={form.tags} onChange={e => setForm({...form, tags: e.target.value})} placeholder="fracciones, sumas, ..." />
              </div>
            </div>
          </div>
          <DialogFooter>
