...
import { useGreeting } from '@/store/useStore'
...
export function DashboardClient({ rawData }) {
    ...
    <PageLayout hideTopBar>
      ...
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-sky-500 via-indigo-500 to-violet-600 text-white p-7 mb-6 shadow-lg">
        ...
        <div className="relative">
          ...
          <h1 className="text-3xl md:text-4xl font-bold mt-1">{greet}, {greetingName} 👋</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
              ...
              <div key={i} className={`p-3 rounded-xl border ${a.priority === 'high' ? 'bg-rose-50 border-rose-100' : 'bg-amber-50 border-amber-100'}`}>
                <div className="flex items-start gap-2.5">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs text-slate-500 mt-1 italic">→ {a.suggested_action}</div>
                  </div>
                </div>
              </div>
            ))}
        ...
        <Card className="border-slate-100 bg-gradient-to-br from-violet-50 to-pink-50 h-full flex flex-col justify-center">
          <CardHeader className="pb-2">
            <CardTitle className="text-base flex items-center gap-2 text-violet-800">
              <Award className="w-4 h-4 text-violet-600" /> Frase del día
            </CardTitle>
          </CardHeader>
          <CardContent>
            <blockquote role="complementary" className="space-y-4">
              <p className="text-lg md:text-xl text-slate-800 italic leading-relaxed font-medium">"{dailyQuote.text}"</p>
              <footer className="text-sm text-slate-700 font-semibold">— {dailyQuote.author}</footer>
            </blockquote>
          </CardContent>
        </Card>
      </div>
        ...
        <CardContent>
          {data.groups.length === 0 ? (
            <div className="text-center py-10">
              ...
              <Button size="sm" onClick={() => router.push('/grupos')} className="bg-sky-600 hover:bg-sky-700 text-white">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {data.groups.slice(0, 6).map(g => (
                <button key={g.id} onClick={() => router.push(`/alumnos?groupId=${g.id}`)} className="text-left p-4 rounded-xl border border-slate-100 hover:border-sky-200 hover:bg-sky-50/30 transition-all group">
                  <div className="flex items-start gap-3">
                    <div className="w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0 font-bold text-white text-base" style={{ background: `linear-gradient(135deg, ${g.color}, ${g.color}dd)` }}>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-xs text-slate-500 mt-1">{g.school_year}</div>
                    </div>
                    <Badge variant="secondary" className="text-2xs flex-shrink-0">{g.student_count || 0} 👥</Badge>
                  </div>
                </button>
              ))}
            </div>
